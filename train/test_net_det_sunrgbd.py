...
def profile_flops_once(model, test_loader, device="cuda"):
    """
    只用 test_loader 的第一批数据做一次 FLOPs/Params 统计。
    注意：遇到自定义 CUDA/pybind11 op 时，thop 可能不会计入或计不准（通常会低估）。
    """
    try:
        from thop import profile
    except Exception as e:
        raise ImportError("Please install thop: pip install thop") from e

    # DataParallel 取实际 module
    m = model.module if isinstance(model, torch.nn.DataParallel) else model
    m.eval()

    data_dicts = next(iter(test_loader))

    # 和你的 test() 保持一致：删掉不需要的 key
    if 'box3d_center' in data_dicts:
        data_dicts.pop('box3d_center')

    # 只搬运 tensor（避免某些非 tensor 字段导致 .cuda() 报错）
    data_dicts_var = {
    ...
    with torch.no_grad():
        flops, params = profile(m, inputs=(data_dicts_var,), verbose=False)


    # 以“单样本”口径输出（thop 统计会包含 batch 维）
    bs = int(data_dicts_var['point_cloud'].shape[0]) if 'point_cloud' in data_dicts_var else 1
    flops_per_sample = flops / max(bs, 1)

    print("========== FLOPs Profile (one batch) ==========")
    print(f"Batch size: {bs}")
    print(f"Params: {params/1e6:.3f} M")
    print(f"FLOPs (batch): {flops/1e9:.3f} G")
    print(f"FLOPs (per-sample): {flops_per_sample/1e9:.3f} G")
    print("===============================================")
...
if __name__ == '__main__':
    ...
    profile_flops_once(model, test_loader, device="cuda")
